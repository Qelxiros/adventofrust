...
use std::{collections::*, fmt::Debug, hash::Hash};
    ...
    loop {
            ...
            .collect_vec()
        {
            match (roles[&value.0], value.1, roles[&value.2]) {
                (
                    Some(AdderType::Carry(a)),
                    "XOR",
                    Some(AdderType::Xor(val)),
                ) if val == a + 1 => {
                ...
                (
                    Some(AdderType::Input(a)),
                    "AND",
                    Some(AdderType::Input(val)),
                ) if val == a && val == 0 => {
                ...
                (
                    Some(AdderType::CarryEasy(a)),
                    "OR",
                    Some(AdderType::CarryHard(val)),
                ) if val == a => {
                    roles.insert(key, Some(AdderType::Carry(val)));
                }
                ...
                _ => match (roles[&value.2], value.1, roles[&value.0]) {
                    ...
                    _ => {
                        ...
                        println!(
                            "{:?}",
                            roles.iter().filter(|(_, v)| v.is_some())
                        );
                        unreachable!()
                    }
                },
