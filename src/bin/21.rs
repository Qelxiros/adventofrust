...
fn get_routes_numeric(start: (usize, usize), end: (usize, usize)) -> Vec<String> {
...
fn obfuscate_first(line: &str, locs: &HashMap<char, (usize, usize)>) -> Vec<String> {
    println!("{line:?}");
    line.chars()
        .tuple_windows()
        .map(|(a, b)| get_routes_numeric(*locs.get(&a).unwrap(), *locs.get(&b).unwrap()))
...
fn part1(input: &str) -> impl Debug {
    ...
    cache = CHARS
        ...
        .map(|(a, b)| {
            (
                (a, b),
                get_routes(*pos_locs.get(&a).unwrap(), *pos_locs.get(&b).unwrap())
                    .into_iter()
                    .map(|s| {
                        let s = format!("A{s}");
                        s.chars()
