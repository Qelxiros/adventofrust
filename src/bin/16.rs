...
use adventofrust::utils::grid_idx;
...
fn part1() {
    ...
    while let Some(State { pos, cost, dir }) = q.pop() {
        ...
        for d in DIRS {
            ...
            if let Ok(x) = <isize as std::convert::TryInto<usize>>::try_into(pos.0 as isize + d.0)
                && let Ok(y) =
                    <isize as std::convert::TryInto<usize>>::try_into(pos.1 as isize + d.1)
            {
                if grid
                    .get(x)
                    .and_then(|l| l.get(y))
