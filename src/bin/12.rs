...
fn neighbors_perim(grid: &Vec<Vec<char>>, pt: (usize, usize)) -> Vec<SidePart> {
    let mut v = Vec::new();
...
fn merge(v: HashSet<SidePart>) -> usize {
    ...
    let (mut a, mut b): (Vec<_>, Vec<_>) = a.into_iter().partition(|s| s.dir > 1);
    let (mut c, mut d): (Vec<_>, Vec<_>) = c.into_iter().partition(|s| s.dir > 1);

    a.sort_unstable_by(|a, b| a.idx.0.cmp(&b.idx.0).then_with(|| a.idx.1.cmp(&b.idx.1)));
    b.sort_unstable_by(|a, b| a.idx.0.cmp(&b.idx.0).then_with(|| a.idx.1.cmp(&b.idx.1)));
    c.sort_unstable_by(|a, b| a.idx.1.cmp(&b.idx.1).then_with(|| a.idx.0.cmp(&b.idx.0)));
    d.sort_unstable_by(|a, b| a.idx.1.cmp(&b.idx.1).then_with(|| a.idx.0.cmp(&b.idx.0)));
...
fn region(grid: &Vec<Vec<char>>, start: (usize, usize)) -> (usize, HashSet<(usize, usize)>) {
    ...
    println!("{}, {area}, {perimeter}", grid[start.0][start.1]);

    (area * perimeter, region)
}

fn part1() {
    let input = include_str!("/home/jeremy/github/adventofrust/input/12");
